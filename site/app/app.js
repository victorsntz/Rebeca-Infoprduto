...
(async function () {
  ...
  async function boot() {
    ...
    if (!st.profile || !st.profile.start_date) return renderOnboarding();
    route();
  }
  window.addEventListener("hashchange", () => { if (st.session && st.profile && st.profile.start_date) route(); });
  ...
  function shell(view, current, accent = "rubi") {
    ...
    app.innerHTML = `<div class="shell c-${accent}">
      <aside class="side-nav">
        ...
        <nav>${nav()}</nav>
        <div class="dayline">${t === 0 ? "Sua travessia começa em " + fmt(st.profile.start_date) : `<b>Dia ${t}</b>de 40 · ${doneCount()} marcados`}<div class="progress"><i style="width:${(doneCount() / TOTAL) * 100}%"></i></div></div>
      </aside>
      <div>
    ...
    const html = head + `
      <div class="two">
        ...
        <div>
          ...
          <div class="row"><label class="label" for="e-oro">Hoje eu oro por <span class="hint">alguém diferente, pelo nome</span></label><input id="e-oro" type="text" class="lined" data-entry="oro" value="${esc(en.oro)}"></div>
      ...
      <div class="two" style="margin-top:1.4rem">
        <div class="stack">
          ...
          <div class="card"><span class="eyebrow">O que é meu nesta prova</span>
            ...
            <div class="field"><label class="label" for="pp">Por quem vou orar nesta prova <span class="hint">uma pessoa só, os 10 dias</span></label><textarea id="pp" data-prep="${pk}.pessoa" rows="2">${esc(p.pessoa)}</textarea></div>
    ...
    shell(`<div class="page-head"><div><span class="eyebrow">Sua conta</span><h1>Conta</h1></div></div>
      <div class="two">
        <div class="card">
          ...
          <div class="field"><label class="label" for="a-date">Meu dia 1</label><input id="a-date" type="date" data-profile="start_date" value="${esc(st.profile.start_date)}"><p class="muted" style="font-size:0.82rem;margin:0.4rem 0 0">Mudar a data desloca todos os dias. Os registros ficam pelo número do dia.</p></div>
